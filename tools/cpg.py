...
def generate_graph(nodes, edges, edge_type_map, ver_edge_type_map):
    index_map = dict()
    index_map_ver = dict()
    all_nodes = set()
    all_ast_edges = []
...
def word2vec(nodes, index_map, graph, wv):
    ...
    for i in index_map:
        ...
        tokens = code_tokenize(node_content)

        w2v_dim = 128
        nrp = np.zeros(w2v_dim)
        for token in tokens:
            try:
                embedding = wv.wv[token]
            except BaseException:
                embedding = np.zeros(w2v_dim)
            nrp = nrp + embedding
        if len(tokens) > 0:
            fnrp = nrp / len(tokens)
        else:
...
def visual():
    ...
    index_map, graph, nodes_num, reduced_num = generate_graph(nodes, edges, edgeType_reduced, ver_edge_type_reduced)
    df_nodes = pd.DataFrame(nodes)
    df_nodes.to_csv("graph/new_nodes.csv")
    df_edges = pd.DataFrame(edges)
    df_edges.to_csv("graph/new_edges.csv")
    if index_map is not None and graph is not None:
...
def main():
    ...
    parser.add_argument('--csv', help='normalized csv files to process',
                        default='graph_data/')
    parser.add_argument('--json_file', help='train and test and valid',
                        default='data/devign/devign.json')
    parser.add_argument('--wv', default='w2v/e5.bin')  # word2vc
    parser.add_argument('--output_dir', default='devign_cpg_c0/')
    ...
    for i, entry in enumerate(data):
        ...
        nodes = read_csv(nodes_path)
        edges = read_csv(edges_path)
        index_map, graph, nodes_num, edges_num = generate_graph(
            nodes, edges, edgeType_reduced, ver_edge_type_reduced)
        if edges_num == True:
        ...
        print(num)
    with open(os.path.join(output_dir, 'devign.json'), 'w') as output_file, \
            open(os.path.join(output_dir, 'devign-num.json'), 'w') as num_file, \
            open(os.path.join(output_dir, 'devign-name.json'), 'w') as name_file, \
            open(os.path.join(output_dir, 'devign-name.json'), 'w') as bad_file_path:
        json.dump(final_data, output_file)
        json.dump(final_num, num_file)
        json.dump(files, name_file)
