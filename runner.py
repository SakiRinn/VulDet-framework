...
class Runner:
    ...
    def train(self, optimizer, dataset, scheduler=None, eval_dataset=None):
        ...
        self.logger.info(f"\tNum examples: {len(dataset)}")
        self.logger.info(f"\tNum epochs: {self.epochs}")
        self.logger.info(f"\tInstantaneous batch size per GPU: {self.per_gpu_batch_size}")
        self.logger.info("\tTotal batch size (w. parallel/distributed training): {}".format(
        ...
        for epoch in tqdm(range(self.cur_epoch, self.epochs + 1)):
            ...
            for step, batch in enumerate(dataloader, start=1):
                ...
                if self.local_rank in [-1, 0]:
                    if self.log_per_steps > 0 and self.cur_step % self.log_per_steps == 0:
                        self.logger.info(f"[epoch {epoch}/step {step}] cur_loss: {round(loss.item(), 4)}, "
                    ...
                    if self.cur_step % self.save_per_steps == 0:
                        self.save_weights(f'{self.model.__class__.__name__}_e{epoch}s{step}',
                                          optimizer, scheduler)
            self.logger.info(f"[epoch {epoch}] Completed. avg_loss: {round(avg_loss, 4)}")
            ...
            if self.local_rank == -1 and eval_dataset is not None:
                ...
                for key, value in results.items():
                    self.logger.info(f"{key} = {round(value, 4)}")
                if results['F1-score'] > best_f1:
                    best_f1 = results['F1-score']
                    self.logger.info(f"Best f1-score: {round(best_f1, 4)}!")
    ...
    def set_logger(self):
        ...
        self.logger.setLevel(logging.INFO if self.local_rank in [-1, 0] else logging.WARN)
    ...
    def save_weights(self, filename, optimizer=None, scheduler=None):
        ...
        self.logger.info(f"Saving the dict of weights to {save_dir}.")
    ...
    def load_weights(self, weight_path, optimizer=None, scheduler=None):
        ...
        try:
            ...
            self.logger.info("Success to load model's weights; "
                             f"current epoch: {self.cur_epoch}, current step: {self.cur_step}.")
        except TypeError:
            self.logger.warning(f"Fail to load model's weights! They are not in {weight_path}.")
        if optimizer is not None:
            try:
                optimizer.load_state_dict(weights['optimizer'])
                self.logger.info("Success to load optimizer's weights.")
            except TypeError:
                self.logger.warning(f"Fail to load optimizer's weights! They are not in {weight_path}.")
        if scheduler is not None:
            try:
                scheduler.load_state_dict(weights['scheduler'])
                self.logger.info("Success to load scheduler's weights.")
            except TypeError:
                self.logger.warning(f"Fail to load scheduler's weights! They are not in {weight_path}.")
