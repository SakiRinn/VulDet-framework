...
class Tokenizer:
    ...
    def extract_path(path):
        return "".join(path.split("/")[:-2])    # last 2 folders

    @staticmethod
    def full_tokenize_cursor(cursor):
        def process_literal(literal):
            cursor_kind = clang.cindex.CursorKind
            kind = literal.cursor.kind
            if kind == cursor_kind.INTEGER_LITERAL or kind == cursor_kind.FLOATING_LITERAL \
                    or re.match('[0-9]+', literal.spelling) is not None:
                return literal.spelling
            elif kind == cursor_kind.IMAGINARY_LITERAL:
                return ["NUM"]
            elif kind == cursor_kind.STRING_LITERAL:
                return ["STRING"]
            else:
                return ["LITERAL"]
    ...
    def split(self, method_only=False):
        ...
        for child in cursor.get_children():
            ...
            extracted_path = self.extract_path(path)

            if ((not method_only and child.kind == cursor_kind.FUNCTION_DECL)
                    or child.kind == cursor_kind.CXX_METHOD) \
                    and extracted_path == self.path:
                tokens = self.full_tokenize_cursor(child)
                results += [tokens]
...
def symbolic_tokenize(code):
    def func_or_keyword(token):
        nonlocal func_count, symbol_table
    ...
    tokens = []
    for token in nltk.word_tokenize(code):
        if token.strip() != '':
    ...
    for i in range(len(tokens)):
        ...
        # functions
            ...
            i += 1
            final_tokens.append(func_or_keyword(token))
        elif token.endswith('('):
            final_tokens.append(func_or_keyword(token[:-1]) + '(')
        elif token.endswith('()'):
            final_tokens.append(func_or_keyword(token[:-2]) + '()')
        # literals
        ...
        # variables
            ...
            if token not in symbol_table.keys():
                ...
                var_count += 1
            final_tokens.append(symbol_table[token])
