...
class AdjacencyList:
    ...
    def __init__(self, num_node, adj_list, device='cuda'):
        self.num_node = num_node
        self.data = torch.tensor(adj_list, dtype=torch.long, device=device)
        self.num_edge = len(adj_list)
...
class GGNNEntry:
    def __init__(self, dataset, feature_repr, label):
        self.dataset = dataset
        assert isinstance(self.dataset, GGNNDataset)
